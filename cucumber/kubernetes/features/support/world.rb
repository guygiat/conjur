  ...
  def pod_certificate
    ...
    while count < retries
      ...
      begin
        ...
        puts "Failed to retrieve client cert with error: #{e}"
        break
      end
    ...
    begin
      get_cert_injection_logs_response = Authentication::AuthnK8s::ExecuteCommandInContainer.new.call(
        ...
        pod_name:          pod_metadata.name,
        container:         "authenticator",
        cmds:              %w(cat /tmp/conjur_set_file_content.log),
        body:              "",
        stdin:             false
      )
      ...
      if !get_cert_injection_logs_response.nil? &&
        ...
        puts "get_cert_injection_logs_response[:stdout] is #{get_cert_injection_logs_response[:stdout]}"
        puts "get_cert_injection_logs_response[:stdout].to_s is #{get_cert_injection_logs_response[:stdout].to_s}"
      else
        puts "Failed to retrieve cert injection logs from container"
      end
    rescue Errors::Authentication::AuthnK8s::ExecCommandError => e
      puts "Failed to retrieve cert injection logs with error: #{e}"
    end
